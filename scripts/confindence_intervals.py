...
# Import spectrum
fs_path = "../data/fs/{}_subsampled.fs".format(args.snps)  # note using subsampled fs here
fs = Spectrum.from_file(fs_path)
ns = fs.sample_sizes
pops = "{}".format(args.snps)
pop_ids = pops.split("-")
# The optimised parameter set
opt = args.opt_params
# The parameter confidence interval txt file which will be output
out_name = "../results/{}_{}_confidence_intervals.txt".format(args.snps, args.model)
...
# Model function
func_ex = Numerics.make_extrap_log_func(model)
...
all_boot = [Spectrum.from_file('../results/bootstraps/{}_bootstrap_vcf_{}.fs'.format(args.snps, i))
...
param_confidence_intervals = Godambe.GIM_uncert(func_ex, PTS, all_boot, opt, fs, multinom=True, eps=args.eps)
...
if args.model == "iso_inbred":
    ...
    with open(out_name, 'a') as file_out:
        file_out.write(f'{args.snps}\t{args.model}\t{low[-1]}\t{low[0]}\t{low[1]}\t{low[2]}\t{low[3]}\t{low[4]}'
                       f'\t{args.eps}\n')
    with open(out_name, 'a') as file_out:
        file_out.write(f'{args.snps}\t{args.model}\t{upp[-1]}\t{upp[0]}\t{upp[1]}\t{upp[2]}\t{upp[3]}\t{upp[4]}'
                       f'\t{args.eps}\n')
elif args.model == "mig_inbred":
    with open(out_name, 'a') as file_out:
        file_out.write('Pops\tModel\ttheta\tnu1\tnu2\tF1\tF2\tm\tT\teps\n')
    with open(out_name, 'a') as fh_out:
        fh_out.write(f'{args.snps}\t{args.model}\t{low[-1]}\t{low[0]}\t{low[1]}\t{low[2]}\t{low[3]}\t{low[4]}\t{low[5]}'
                     f'\t{args.eps}\n')
    with open(out_name, 'a') as fh_out:
        fh_out.write(f'{args.snps}\t{args.model}\t{upp[-1]}\t{upp[0]}\t{upp[1]}\t{upp[2]}\t{upp[3]}\t{upp[4]}\t{upp[5]}'
                     f'\t{args.eps}\n')
else:
