#!/bin/bash --login
#SBATCH --job-name="confidence_group1-group2_hetsym"
#SBATCH --nodes=1
...
#SBATCH -o confidence-group1-group2_hetsym_%A.o
#SBATCH -e confidence-group1-group2_hetsym_%A.e
...
results_file="../results/proj_hetsym_best_models_dadi_params.csv"
...
for i in 1 0.01 0.001 0.0001 0.00001 0.000001; do
    echo "Running confidence intervals for hetsym with epsilon ${i} for ${pop}"
    python confidence_intervals.py "../data/fs/${pop}_projected0.8.fs" "../data/fs/projected_${pop}/" \
        GIM  1het_sym ${i} -o ${opt}
done
