...
PATH="/scratch/user/uqkprat2/analysis/kp_dadi"

def main(snps, model, masked, folds, int_params, PTS, method=None, path=None):
    # Import and define data constants
    if method == "subsample":
        data = dadi.Spectrum.from_file('{}/data/fs/{}_subsampled.fs'.format(PATH, snps))
    elif method == "projection":
        data = dadi.Spectrum.from_file('{}/data/fs/{}_projected.fs'.format(PATH, snps))
    else:
        data = dadi.Spectrum.from_file('{}/data/fs/{}.fs'.format(PATH, snps))
    pops = "{}".format(snps)
    pop_ids = pops.split("-")
