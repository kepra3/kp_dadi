...
if args.method == "subsample":
    ...
    fs.to_file("../data/fs/{}_subsampled.fs".format(args.snps))
elif args.method == "projection":
    dd = dadi.Misc.make_data_dict_vcf(snp_path, pop_path)
    fs = dadi.Spectrum.from_data_dict(dd, pop_ids=pop_ids, projections=proj, polarized=False)
    fs.to_file("../data/fs/{}_projected.fs".format(args.snps))

# Printing out stats for the fs
print("The datafile will be named {}".format(args.snps))
print("\n* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *\n")
...
with open(stats_out_name, "a") as stats_out:
    stats_out.write("{0}\t{1}\t{2}\t{3}\n".format(args.snps, fs.sample_sizes, np.around(fs.S(), 2),
                                                  np.around(fs.Fst(), 2)))
...
if args.masked == "yes":
    ...
    with open(stats_out_name, "a") as stats_out:
        stats_out.write("{0}\t{1}\t{2}\t{3}\n".format("{}_masked".format(args.snps), fs.sample_sizes,
